...
function findingsEngine_processLastRow() {
  try {
    ...
    var result = findingsEngine_extract_(data, "IAG-PHC-TECH");
    auditEngine_logEvent("FINDINGS_EXTRACT", "Auto-extract findings initiated", "processLastRow", { result: result });
    return result;
  } catch(e) {
    auditEngine_logError("findingsEngine_processLastRow", e);
...
function findingsEngine_testLastRow() {
  try {
    ...
    auditEngine_logError("findingsEngine_testLastRow", e);
  ...
  try {
    ...
    allFindings.forEach(function(f) {
      try {
        ...
        auditEngine_logError("findingsEngine_extract_ row", rowErr);
      }
    });

    auditEngine_logEvent("FINDINGS_EXTRACTED", "استخرج " + written + " مخالفة من " + meta.unitName, "", { written: written, skipped: skipped });

    try { SpreadsheetApp.flush(); } catch(_) {}

    // ── استدعاء CAREngine مباشر ──
    try {
      if (typeof carEngine_processNewFindings_direct_ !== "undefined") {
        carEngine_processNewFindings_direct_();
      }
    } catch (carErr) {
      auditEngine_logError("findingsEngine → carEngine_direct", carErr);
    }

    return { ok: true, extracted: written, skipped: skipped, unit: meta.unitName };
  } catch(e) {
    auditEngine_logError("findingsEngine_extract_", e);
    return { ok: false, error: e.message };
  }
}
...
function findingsEngine_extractMeta_(data) {
  try {
    ...
    auditEngine_logError("findingsEngine_extractMeta_", e);
    return { officer: "غير محدد", unitName: "غير محدد", adminArea: "", visitDate: new Date() };
  }
}
...
function findingsEngine_collectViolations_(data, meta, codesMap) {
  try {
    ...
    auditEngine_logError("findingsEngine_collectViolations_", e);
    return [];
  }
}
...
function findingsEngine_classify_(text) {
  try {
    ...
    auditEngine_logError("findingsEngine_classify_", e);
    return { classification: "🟢 مؤشر", severity: "منخفض" };
  }
}
...
function findingsEngine_buildHeaderMap_(sheet) {
  ...
  try {
    ...
    auditEngine_logError("findingsEngine_buildHeaderMap_", e);
  }
  return map;
}
...
function findingsEngine_loadCodesMap_() {
  ...
  try {
    ...
    auditEngine_logError("findingsEngine_loadCodesMap_", e);
  }
  return map;
}
...
function findingsEngine_lookupCode_(violationText, codesMap) {
  try {
    ...
    auditEngine_logError("findingsEngine_lookupCode_", e);
    return null;
  }
}
...
function findingsEngine_buildDedupSet_(findingsSh, unitName, visitDate) {
  ...
  try {
    ...
    auditEngine_logError("findingsEngine_buildDedupSet_", e);
  }
  return dedupSet;
}
...
function findingsEngine_checkRepeat_(findingsSh, unitName, findingCode) {
  ...
  try {
    ...
    auditEngine_logError("findingsEngine_checkRepeat_", e);
  }
  return result;
}