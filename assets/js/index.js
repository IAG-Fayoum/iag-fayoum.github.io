...
if (loginForm) {
    loginForm.addEventListener('submit', async (e) => {
        ...
        try {
            const result = await IAGApi.login(mobile, pin);

            if (!result.ok) {
                throw new Error(result.error || 'بيانات غير صحيحة');
            }

            // Persist session — _normalize() strips envelope fields, result.data is the user object
            IAGSession.setUser(result.data);
            redirectUser(result.data);
